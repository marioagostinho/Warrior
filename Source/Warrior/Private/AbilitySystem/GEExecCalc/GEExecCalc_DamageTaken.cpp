	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TPair<FGameplayTag, float>& TagMagnitude : EffectSpec.SetByCallerTagMagnitudes)
	{
		...
		{
			BasedDamage = TagMagnitude.Value;
		}
	...
	if (UsedLightAttackComboCount != 0)
	{
		...
		BasedDamage *= DamageIncreasePercentLight;
	}

	if (UsedHeavyAttackComboCount != 0)
	{
		const float DamageIncreasePercentHeavy = UsedHeavyAttackComboCount * 0.15f + 1.f;
		BasedDamage += DamageIncreasePercentHeavy;
	}

	if (const float FinalDamageDone = BasedDamage * SourceAttackPower / TargetDefensePower; FinalDamageDone > 0.f)
	{
